...
const NOTIFICATION_TYPES = {
  insider: [
    { id: 'pitch_received', label: 'Pitch received', sub: 'When a seeker pitches you for a referral', default: 'inapp' },
    { id: 'match_accepted', label: 'Match accepted', sub: 'When you accept a pitch and a chat opens', default: 'inapp' },
    { id: 'pipeline_updates', label: 'Pipeline updates', sub: 'When a seeker updates their interview stage', default: 'both' },
    { id: 'messages', label: 'New messages', sub: 'When a matched seeker sends you a message', default: 'inapp' },
    { id: 'barakah_points', label: 'Barakah Points', sub: 'When you earn points for a referral milestone', default: 'both' },
  ],
  seeker: [
    { id: 'pitch_accepted', label: 'Pitch accepted', sub: 'When an insider accepts your pitch', default: 'both' },
    { id: 'pitch_declined', label: 'Pitch declined', sub: 'When an insider declines your pitch', default: 'both' },
    { id: 'messages', label: 'New messages', sub: 'When an insider sends you a message', default: 'inapp' },
  ],
}
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (existing) {
        // Map DB row to settings object
        const mapped = {}
        NOTIFICATION_TYPES[role].forEach(n => {
          mapped[n.id] = existing[n.id] || n.default
        })
        setSettings(mapped)
      } else {
        // Use defaults — row doesn't exist yet
        const defaults = {}
        NOTIFICATION_TYPES[role].forEach(n => {
  ...
  async function handleSave() {
    ...
    Object.entries(settings).forEach(([key, val]) => { row[key] = val })
    ...
    <div style={{ minHeight: '100vh', background: '#f5f4f0', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <div style={{ padding: '16px' }}>
        <div style={{ background: '#E1F5EE', border: '0.5px solid #9FE1CB', borderRadius: '10px', padding: '12px 14px', marginBottom: '16px', fontSize: '13px', color: '#085041', lineHeight: '1.6' }}>
          {userRole === 'insider'
            ? "Pitch notifications are off by default — check your feed when you're ready to review pitches."
